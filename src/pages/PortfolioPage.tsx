    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-200 selection:text-indigo-900 pb-32">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
        ...
        <div className="hidden sm:flex items-center gap-8 text-sm font-semibold text-slate-600">
          <button onClick={() => scrollTo("about")} className="hover:text-indigo-600 transition">{t("portfolio.aboutMe")}</button>
          <button onClick={() => scrollTo("skills")} className="hover:text-indigo-600 transition">{t("portfolio.capabilities")}</button>
          <button onClick={() => scrollTo("experience")} className="hover:text-indigo-600 transition">{t("portfolio.experience")}</button>
          {user?.email === portfolio.email && (
            <Link
              to={`/register?edit=${portfolio.slug}`}
              className="px-5 py-2 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition font-bold shadow-md hover:-translate-y-0.5 hover:shadow-lg"
            >
        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/" className="text-xs font-semibold text-slate-500 hover:text-indigo-600 transition">
      ...
      <header className="pt-32 pb-20 px-6 max-w-4xl mx-auto flex flex-col-reverse md:flex-row items-center gap-12 md:gap-8 justify-between">
        <div className="flex-1 animate-[fadeUp_0.8s_ease]">
          ...
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight leading-[1.1] text-slate-900">
            ...
            <span className="text-indigo-600">{portfolio.title}</span>
          </h1>
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            <a
              ...
              className="px-8 py-3.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 text-white font-bold hover:from-indigo-600 hover:to-indigo-700 transition shadow-md hover:-translate-y-0.5 hover:shadow-lg"
            >
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="rounded-3xl bg-white border border-slate-200 p-8 shadow-sm flex flex-col justify-center gap-4">
              <div>
                ...
                <a href={`mailto:${portfolio.email}`} className="text-lg font-bold text-slate-900 hover:text-indigo-600 transition">{portfolio.email}</a>
              </div>
              <div className="w-full h-px bg-slate-100" />
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-widest font-bold mb-1">{t("portfolio.phone")}</p>
                <a href={`tel:${portfolio.phone}`} className="text-lg font-bold text-slate-900 hover:text-indigo-600 transition">{portfolio.phone || t("portfolio.notProvided")}</a>
              </div>
            </div>
          </div>
        </div>
      ...
      <footer className="mx-auto max-w-4xl px-6 pt-24 pb-12 text-center mt-12">
        ...
        <a
          href={`mailto:${portfolio.email}`}
          className="inline-flex items-center justify-center px-8 py-3.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 text-white font-bold hover:from-indigo-600 hover:to-indigo-700 transition shadow-md hover:-translate-y-0.5 hover:shadow-lg"
        >
        ...
        <div className="mt-20 flex flex-col md:flex-row items-center justify-between text-sm font-medium text-slate-500">
          ...
          <div className="mt-4 md:mt-0">
            <Link to="/" className="hover:text-indigo-600 transition">{t("portfolio.poweredBy")}</Link>
          </div>
        </div>
      </footer>
